...
<section class="home-slider-container">
	...
	function slide_with_thumb_nav( $slide ) {
		...
		if ( ! empty( $slide->img->thumb ) ) {
			$rt = '
	<li class="slide" data-thumb="' . str_replace( array( 'https://', 'http://' ), array( '//', '//' ), $slide->img->thumb ) . '" data-thumb-alt="' . sprintf( __( 'Advance to the %s slide' ), strip_tags( apply_filters( 'the_title_attribute', $slide->caption->title ) ) ) . '">
		<article class="slide-content">';
		} else {
			$rt = '
	<li class="slide">
		...
		$rt .= '
			<img src="' . str_replace( array( 'https://', 'http://' ), array( '//', '//' ), $slide->img->src ) . '" alt="' . strip_tags( $slide->img->alt ) . '" />';
